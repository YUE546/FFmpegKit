...
* **智能加速**：自动检测 FFmpeg 内核对 **NVIDIA (NVENC)**、**Intel (QSV)**、**AMD (AMF)** 的支持情况，并根据环境动态调整 UI 选项；硬件解码与硬件编码器独立控制，支持「软编+硬解」「全硬编」等多种组合。
* **双模编码**：转换功能支持「质量模式」（CRF/QP）与「码率模式」（平均码率 + 可选上限）二选一，适配不同输出场景。
...
1.  **音视频格式转换**：支持主流封装格式（MP4/MKV/MP3/M4A等）互转，内置多种编码器预设；支持质量模式（CRF/QP）与码率模式（平均码率 + 可选上限）二选一。
...
### 硬件解码与视频编码器的关系
转换功能中「硬件解码加速」复选框与「视频编码器」下拉框是两个独立但联动的控件：
* **硬件解码加速**：控制输入端的 `-hwaccel` 参数，决定如何**读取/解码**源视频。
* **视频编码器**：控制输出端用哪个编码器**生成**目标视频（如 libx264/h264_nvenc/h264_amf/h264_qsv 等）。

二者组合规则：
| 选中编码器 | 硬件解码加速复选框 | 实际解码方式 |
|---|---|---|
| 硬件编码器（如 h264_nvenc） | 任意（UI 会自动勾上） | **强制启用**，按编码器 GpuType 选 cuda/auto |
| 软件编码器或未指定 | 勾选 | 启用，按全局 `DefaultGPU` 选 cuda/auto |
| 软件编码器或未指定 | 未勾选 | 不启用，纯软解 |

> 💡 「软件编码器 + 硬件解码加速」是合法组合；但不存在「硬件编码器 + 纯软解」的组合。

### 编码模式
转换功能提供两种互斥的编码模式（RadioButton 二选一）：
* **质量模式**：使用 CRF（libx264/libx265）或 QP（NVENC）/ CQ（AMF）/ CQP（QSV）指定质量值（0-51，默认 23），适合追求画质的场景。
* **码率模式**：通过 `-b:v` 指定平均码率（kbps），适合对体积有明确要求的场景。

两种模式下均可选填「码率上限」，生成 `-maxrate` + `-bufsize` 参数（bufsize 默认为 maxrate 的 2 倍）实现 VBR 约束。
...
- **持续集成**：GitHub Actions（Windows + MSBuild Release）
- **作者**：by Candura
- **版本**：2026.07 Stable
- **开源协议**：GPL v3.0
